...
```{r}
# Remove SSC from channels of interestsince it needs its own transformation
channels_to_transform <- channels_of_interest[-1]
...
for (file in files){
  ...
  write.FCS(ff_t, file.path(dir_prepr, basename(file)))
}
```
