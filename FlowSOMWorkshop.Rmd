...
Computational cytometry
  - Data
      ...
      + manualGating_Live.wsp
      + manualGating.pdf
      + Populations_and_markers.xlsx
...
```{r}
plotDens(ff, GetChannels(ff, c("CD3", "CD19")))
```
...
print(GetMarkers(ff, channels_of_interest))
```
...
files_batch <- list.files(batch_data, pattern = ".fcs") # already preprocessed and QC filtered

dates <- c()
for (file in file.path(batch_data, files_batch)){
  ff <- read.FCS(file)
  dates <- c(dates, ff@description$`$DATE`)
}
dates
dates <- as.Date(dates, "%d-%b-%Y") # convert "24-FEB-2017" to date R can understand
dates
```

Comparison plot between all the files
```{r}
fs_batch <- read.flowSet(file.path(batch_data, files_batch)[order(dates)]) # read in files in order of the date

plot_aggregate(input = fs_batch, 
               labels = rep(c("1", "2"), times = c(5, 5)),
               sample_names = dates[order(dates)],
               output_image = file.path(batch_dir, "aggregate.png"))
```

We have 2 batches, normalization is needed
```{r}
channels_to_norm <- colnames(ff)[c(1, 5:13)]
...
model <- CytoNorm.train(files = file.path(batch_data, files_batch[order(dates)]),
                        labels = rep(c("1", "2"), times = c(5, 5)), # Each file unique label for per file normalization
                        channels = channels_to_norm,
                        transformList = NULL,
...
CytoNorm.normalize(model = model,
                   files = file.path(batch_data, files_batch[order(dates)]),
                   labels = rep(c("1", "2"), times = c(5, 5)),
                   transformList = NULL,
...
fs_CytoNorm <- read.flowSet(list.files("Workshop_batch/CytoNorm",
                                       pattern = ".fcs",
                                       full.names = T)[order(dates)])

plot_aggregate(input = fs_CytoNorm, 
               labels = rep(c("1", "2"), times = c(5, 5)),
               output_image = file.path(batch_dir, "aggregate_CytoNorm.png"))
```
...
```{r}
wsp_file <- file.path(dir, "manualGating_Live.wsp")
...
for (file in files){
  ...
  PlotStars(UpdateNodeSize(fsom_tmp$FlowSOM),
            main=file)
}
